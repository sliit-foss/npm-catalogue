export const getPaginatedResults = async (queryFunction, organizations, filters, owner, repository) => {
  const finalResults = [];
  const results = await queryFunction(organizations, filters, "", owner, repository);
  ...
  while (!lastPage && pages < pageLimit) {
    const nextPageResults = await queryFunction(organizations, filters, cursor, owner, repository);
