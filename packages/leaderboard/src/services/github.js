...
const _searchQuery = (searchEntity, filters, afterCursor,) => {
  ...
  return `
  search(first: ${pageSize}, type: ISSUE, query: "${searchEntity} is:pr is:merged ${filters.between ? `created:${filters.between}` : ""} ${filters.label ? `label:${filters.label}` : ""}", ${afterCursor != "" ? `after: "${afterCursor}"` : ""}) {
    issueCount
    edges {
      node {
        ... on PullRequest {
